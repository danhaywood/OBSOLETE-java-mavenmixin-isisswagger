...
echo ""
mvn clean package -Dskip.danhaywood-mavenmixin-isisswagger -o >/dev/null
if [ $? != 0 ]; then
    echo "... failed" >&2
    exit 1
fi
...
echo ""
mvn clean deploy -Pdanhaywood-mavenmixin-sonatyperelease -Dskip.danhaywood-mavenmixin-isisswagger -Dpgp.secretkey=keyring:id=$KEYID -Dpgp.passphrase="literal:$PASSPHRASE"
if [ $? != 0 ]; then
    echo "... failed" >&2
    exit 1
fi
